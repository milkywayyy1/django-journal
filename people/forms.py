...
class UserCreateForm(forms.ModelForm):
    class Meta:
        ...
        fields = [ 'username', 'email', 'password', 'first_name', 'last_name', 'middle_name',
                  'birth_date', 'sex', 'photo', 'description', 'face_id', 'user_status']
        widgets = {'username': forms.TextInput(attrs={'placeholder': 'Имя пользователя'}),
                   ...
                   'face_id': forms. NumberInput(attrs={'placeholder': 'Номер пользователя'}),
                   'user_status': forms.Select(attrs={'placeholder': 'Статус пользователя'})
                   }
